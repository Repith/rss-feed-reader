...
export class FeedService {
  ...
  async updateFeed(id: string, data: Partial<Feed>): Promise<Feed | null> {
  ...
  async refreshFeed(id: string): Promise<Feed | null> {
    ...
    const { articles } = await this.rssParserService.parseFeed(feed.url);
    ...
    return this.feedRepository.update(id, { lastFetched: new Date() });
  }
